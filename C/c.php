...
$keyb(密匙b)，
//解密时会通过这个密匙验证数据完整性
// 如果是解码的话，会从第$ckey_length位开始，因为密文前$ckey_length位保
...
// 用固定的算法，打乱密匙簿，增加随机性，好像很复杂，实际上对并不会增加
...
0) && substr($result, 10, 16) == substr(md5(substr($result, 26) . 
...
// 把动态密匙保存在密文里，这也是为什么同样的明文，生产不同密文后能解密

的原因
// 因为加密后的密文可能是一些特殊字符，复制过程可能会丢失，所以用base64
...
//函数authcode($string, $operation, $key, $expiry)中的$string：字符串，

明文或密文；$operation：DECODE表示解密，其它表示加密；$key：密匙；
...
{ //标签云
    ...
    foreach ($a as $v) {
       ...
       $bqy[]= '<a href="/index.php?bq='.$v.'" title="'.$v.'">'.
...
title="'.bm($v).'">'.bm($v).'</a>';
        ...
        if(bh(bm($v),$s)) { 
             $ss[] = '<a target="_blank" href="'.bm($v).'" title="'.bm
...
($v).'">'.bm($v).'</a>';
              ...
              $ss[] = '<a target="_blank" href="'.bm($v).'" title="'.
...
$extension = end($temp);
  ...
  if (($_FILES["file"]["size"][$k] < $size)&& in_array($extension, 

$allowedExts)) {  
     if (file_exists($dir. bm($_FILES["file"]["name"][$k])))
 {
	 echo "<script> alert('".$_FILES["file"]["name"][$k] ."文件已经

存在！请修改文件名');location.href='".$_SERVER["HTTP_REFERER"]."'; 
 ...
 {
 tj($dir. bm($_FILES["file"]["name"][$k]),dk($_FILES["file"]

["tmp_name"][$k]) )  ;
  echo "<script> alert('文件成功上传至:". $dir. $_FILES["file"]

["name"][$k]."');location.href='".$_SERVER["HTTP_REFERER"]."'; 
...
if(isset($_POST['tj'])){
	...
	tj("up/".$_FILES['file']['name'][$k],dk($_FILES['file']
...
function setWater($imgSrc,$markImg,$markText,$TextColor,$markPos,

$fontType,$markType)//图片水印
{
    
    
 /**
$imgSrc：目标图片，可带相对目录地址，
$markImg：水印图片，可带相对目录地址，支持PNG和GIF两种格式，如水印图片

在执行文件mark目录下，可写成：mark/mark.gif
$markText：给图片添加的水印文字
$TextColor：水印文字的字体颜色
...
$markType：图片添加水印的方式，img代表以图片方式，text代表以文字方式添
...
加水印
  */
  ...
  if(!strcmp($markType,"text"))
  {
    ...
    imagettftext($dst_img, $fontSize, 0, $x, $y, $color, $fontType,
...
function slt($big_img, $width, $height, $small_img) {//原始大图地址，缩
...
imagecopyresampled($tn, $im, 0, 0, 0, 0, $width, $height, $src_W, 
...
{ 
$encode = mb_detect_encoding($s, array("ASCII", "UTF-8", "GB2312", 
